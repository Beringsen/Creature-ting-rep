...
float  ballX             = 100 ,  ballY             = 100; //heroppe har jeg ændret alle "int" til "float" for at sørge for at øjnene bevæger sig reletivt i stedet
float   eyeLeftRelativeX  = 5   ,  eyeLeftRelativeY  = 5;
float  eyeRightRelativeX = 18  ,  eyeRightRelativeY = 5;   
...
void draw() {                         
  ...
  ellipse(eyeRightRelativeX + ballX, eyeRightRelativeY + ballY+ballwalk, eyeSize,eyeSize); //jeg har tilføjet ballwalk til alle positionerne her da det er funktionen sin(ballX*0.5)*10 som gør at boldene hopper
}
